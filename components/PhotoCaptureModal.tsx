import React, { useState, useEffect } from 'react';
  ...
  const PhotoCaptureModal: React.FC<PhotoCaptureModalProps> = ({ isVisible, onClose, onConfirm }) => {
    ...
    const formatTimestamp = (date: Date) =>
      ...
      const stampPhotoWithTimestamp = async (uri: string): Promise<string> => {
        const timestampDate =
          location && location.timestamp instanceof Date ? location.timestamp : new Date();
        const text = formatTimestamp(timestampDate);
    
        try {
          const resultPath = await ImageMarker.markText({
            backgroundImage: {
              src: uri,
              scale: 1,
            },
          ...
          // Normalize to a proper URI for React Native
          if (
            resultPath &&
            !resultPath.startsWith('file://') &&
            !resultPath.startsWith('content://') &&
            !resultPath.startsWith('http') &&
            !resultPath.startsWith('data:')
          ) {
          ...
          console.error('Error adding timestamp to photo:', error);
          return uri;
        }
      };
    ...
    const handleTakePhoto = async (isBackPhoto: boolean = false) => {
      ...
      try {
        ...
        if (response.assets?.[0]?.uri) {
          ...
          const stampedUri = await stampPhotoWithTimestamp(originalUri);
      
          if (isBackPhoto) {
            setPhotoBack(stampedUri);
          } else {
            setPhotoFront(stampedUri);
          }
        }
      } catch (error) {
